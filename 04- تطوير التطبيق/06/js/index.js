...
myApp.onPageBeforeAnimation('category', function(page) {
    ...
    db.transaction(function(tx) {
        tx.executeSql('SELECT * FROM Category', [], function(tx, rs) {
            for (var i = 0; i < rs.rows.length; i++) {
                var category = '<li id="' + rs.rows.item(i).category_id + '"><div class="item-content" ><div class="item-media category_arrow"><i class="icon f7-icons">arrow_left</i></div><div class="item-inner" >' + '<div class="item-title">' + rs.rows.item(i).category_name + '</div>' + '<div class="item-after delete_category">' + '<i class="icon f7-icons">delete_round</i>' + '</div></div></div></li>';
